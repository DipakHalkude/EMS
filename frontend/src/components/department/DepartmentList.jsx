    ...
    try {
      ...
      if (response.data.success) {
        ...
        const data = response.data.departments.map((dep) => ({
          ...
          action: (
            <DepartmentButtons _id={dep._id} onDepartmentDelete={fetchDepartments} />
          ),
        }));
      ...
      alert(error.response?.data?.error || "Server error while fetching departments");
    } finally {
      setDepLoading(false);
    }
  };
        ...
        <div className="mt-5 min-h-fit bg-gradient-to-br from-gray-900 via-blue-900 to-purple-900 p-6 relative overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-gradient-to-br from-gray-800/40 to-blue-800/40 backdrop-blur-lg rounded-2xl border border-cyan-500/30 p-6 shadow-lg shadow-cyan-500/20 relative z-10 mb-6">
            ...
            <Link
              ...
              className="px-6 py-3 bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white rounded-xl font-semibold transition-all duration-300 transform hover:scale-105 hover:shadow-lg hover:shadow-cyan-500/30 text-center min-w-[200px]"
            >
              Add New Department
            </Link>
          </div>
          ...
          <div className="relative z-10 bg-gradient-to-br from-gray-800/40 to-blue-800/40 backdrop-blur-lg rounded-2xl border border-cyan-500/30 shadow-lg shadow-cyan-500/20 overflow-hidden">
            <DataTable
              ...
              customStyles={{
                ...
                head: { style: { backgroundColor: "rgba(17,24,39,0.6)", color: "#67e8f9", fontSize: "1rem", fontWeight: "bold", backdropFilter: "blur(10px)" } },
                headRow: { style: { borderBottom: "1px solid rgba(6,182,212,0.3)", backdropFilter: "blur(10px)" } },
                headCells: { style: { padding: "16px", textTransform: "uppercase", letterSpacing: "0.05em" } },
                cells: { style: { backgroundColor: "rgba(17,24,39,0.4)", color: "white", padding: "16px", borderBottom: "1px solid rgba(6,182,212,0.1)", backdropFilter: "blur(10px)", textAlign: "center" } },
                rows: { style: { backgroundColor: "rgba(17,24,39,0.4)", "&:hover": { backgroundColor: "rgba(6,182,212,0.1)" } } },
                pagination: { style: { backgroundColor: 'rgba(17,24,39,0.6)', color: 'white', borderTop: '1px solid rgba(6,182,212,0.3)', backdropFilter: 'blur(10px)' }, pageButtonsStyle: { color: '#67e8f9', fill: '#67e8f9', '&:disabled': { color: 'rgba(103,232,249,0.5)' } } },
              }}
              pagination
              highlightOnHover
            />
